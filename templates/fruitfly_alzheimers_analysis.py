...
def run_analysis(model, config, path_manager, pipeline):
    ...
    try:
        ...
        df['prediction_error'] = pd.to_numeric(df['predicted_age']) - pd.to_numeric(df['actual_age'])
...
def genotype_comparison_analysis(df, output_dir):
    ...
    try:
        ...
        for genotype in df['genotype'].unique():
            ...
            print(f"   {genotype}: {mean_error:+.2f} ± {std_error:.2f} days (n={n_samples}, {older_pct:.1f}% older)")
...
def age_specific_analysis(df, output_dir):
    ...
    try:
        ...
        for age in sorted(df['actual_age'].unique()):
            ...
            print(f"   Age {age}: {mean_error:+.2f} ± {std_error:.2f} days (n={n_samples})")
...
def create_comprehensive_visualizations(df, output_dir):
    ...
    try:
        ...
        fig, axes = plt.subplots(2, 2 if has_genotype else 2, figsize=(15, 12) if has_genotype else (12, 12))
        if not has_genotype:
            axes = np.append(axes, [None])  # Pad for consistent indexing

        # 1. Prediction error distribution
        axes[0,0].hist(df['prediction_error'], bins=30, alpha=0.7, color='skyblue', edgecolor='black')
        axes[0,0].axvline(0, color='red', linestyle='--', label='Perfect Prediction')
        axes[0,0].axvline(df['prediction_error'].mean(), color='orange', linestyle='-', label=f'Mean = {df["prediction_error"].mean():.2f}')
        ...
        axes[0,1].scatter(df['actual_age'], df['predicted_age'], alpha=0.6, color='blue')
        min_age, max_age = df['actual_age'].min(), df['actual_age'].max()
        axes[0,1].plot([min_age, max_age], [min_age, max_age], 'r--', alpha=0.5, label='Perfect Prediction')
        ...
        axes[1,0].errorbar(age_means.index, age_means.values, yerr=age_stds.values,
...
def generate_summary_report(df, output_dir):
    ...
    try:
        ...
        summary = {
            ...
            'accelerated_aging_evidence': mean_error > 1.0,  # More than 1 day older on average
            'interpretation': {
                'aging_acceleration': "Strong evidence" if mean_error > 2.0 else "Moderate evidence" if mean_error > 1.0 else "Weak evidence" if mean_error > 0 else "No evidence",
                'biological_significance': "Model detects disrupted aging patterns in Alzheimer's flies" if mean_error > 1.0 else "No clear aging acceleration detected"
            }
        }
        ...
        if 'genotype' in df.columns:
            ...
            for genotype in df['genotype'].unique():
                ...
                genotype_summary[genotype] = {
                    ...
                    'older_percentage': float((genotype_data['prediction_error'] > 0).mean() * 100)
                }
        ...
        print(f"   Biological interpretation: {summary['interpretation']['biological_significance']}")
