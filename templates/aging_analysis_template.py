...
def aging_acceleration_analysis(predictions_df, output_dir):
    ...
    try:
        if (
            "true_age" not in predictions_df.columns
            or "predicted_age" not in predictions_df.columns
        ):
            print("   ⚠️ Age columns not found, skipping aging acceleration analysis")
            return

        # Calculate aging acceleration
        predictions_df["aging_acceleration"] = (
            predictions_df["predicted_age"] - predictions_df["true_age"]
        )
        ...
        print(
            f"   Accelerated samples: {acc_stats['accelerated_samples']} ({acc_stats['accelerated_samples'] / len(predictions_df) * 100:.1f}%)"
        )
        print(
            f"   Decelerated samples: {acc_stats['decelerated_samples']} ({acc_stats['decelerated_samples'] / len(predictions_df) * 100:.1f}%)"
        )
        ...
        predictions_df.to_csv(
            output_dir / "predictions_with_acceleration.csv", index=False
        )
    ...
    try:
        ...
        for group in age_bins.categories:
            ...
            if len(group_data) > 0 and "predicted_age" in predictions_df.columns:
                mae = np.mean(
                    np.abs(group_data["true_age"] - group_data["predicted_age"])
                )
                rmse = np.sqrt(
                    np.mean((group_data["true_age"] - group_data["predicted_age"]) ** 2)
                )
                r2 = (
                    stats.pearsonr(group_data["true_age"], group_data["predicted_age"])[
                        0
                    ]
                    ** 2
                )
                ...
                print(
                    f"   {group}: MAE={mae:.2f}, RMSE={rmse:.2f}, R²={r2:.3f} (n={len(group_data)})"
                )
        ...
        if group_stats:
            ...
            axes[0].bar(
                group_stats_df["age_group"], group_stats_df["mae"], color="lightcoral"
            )
            ...
            axes[1].bar(
                group_stats_df["age_group"], group_stats_df["rmse"], color="lightblue"
            )
            ...
            axes[2].bar(
                group_stats_df["age_group"], group_stats_df["r2"], color="lightgreen"
            )
            ...
            plt.tight_layout()
            plt.savefig(
                output_dir / "age_group_performance.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
