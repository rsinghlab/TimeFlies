...
def test_timeflies_with_correct_data_format():
    ...
    with patch("common.utils.path_manager.PathManager") as mock_pm_class:
        ...
        assert total_cells <= adata.n_obs, f"Split cells ({total_cells}) shouldn't exceed original ({adata.n_obs})"
    ...
    with patch("common.utils.path_manager.PathManager") as mock_pm_class:
        ...
        X_train = processed.X.toarray() if hasattr(processed.X, 'toarray') else processed.X
        ...
        print(f"  📊 Training data: {X_train.shape[0]} samples, {n_features} features, {n_classes} classes")
        ...
        assert all(isinstance(p, (int, np.integer)) for p in predictions), "Predictions should be integers"
        assert all(0 <= p < n_classes for p in predictions), "Predictions should be valid class indices"
        
        # Test probabilities
        probabilities = model.predict_proba(X_train[:3])
        assert probabilities.shape == (3, n_classes), f"Expected shape (3, {n_classes}), got {probabilities.shape}"
