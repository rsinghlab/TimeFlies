...
class TestCLIWorkflowIntegration:
    ...
    def test_project_switching_real_workflow(self):
        ...
        # Test aging project
        with patch('shared.core.active_config.get_active_project', return_value='fruitfly_aging'):
            config_manager = get_config_for_active_project('default')
            config = config_manager.get_config()
        ...
        try:
            with patch('shared.core.active_config.get_active_project', return_value='fruitfly_alzheimers'):
                config_manager = get_config_for_active_project('default')
                config = config_manager.get_config()
                assert config is not None
