...
class CNNModel(BaseModel):
    ...
    def build(self, input_shape: Tuple[int, ...], num_classes: int) -> None:
        ...
        try:
            ...
            strides = getattr(model_config, 'strides', [1, 1, 1])
            paddings = getattr(model_config, 'paddings', ['same', 'same', 'same'])
            ...
            self.model.add(Conv1D(
                ...
                strides=strides[0] if len(strides) > 0 else 1,
                padding=paddings[0] if len(paddings) > 0 else 'same',
                activation=activation,
                input_shape=(1, input_shape[0])  # (sequence_length=1, features)
            ))
            ...
            for i in range(1, len(filters)):
                self.model.add(Conv1D(
                    ...
                    strides=strides[i] if i < len(strides) else strides[-1] if len(strides) > 0 else 1,
                    padding=paddings[i] if i < len(paddings) else paddings[-1] if len(paddings) > 0 else 'same',
                    activation=activation
                ))
...
class MLPModel(BaseModel):
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        try:
            ...
            raise ModelError(f"Failed to train MLP model: {e}")
