...
from ..data.setup import DataSetupManager
...
def run_system_tests(args) -> int:
    ...
    config_manager_level = logging.getLogger('timeflies.projects.fruitfly_aging.core.config_manager').level
    ...
    logging.getLogger('timeflies.projects.fruitfly_aging.core.config_manager').setLevel(logging.ERROR)
                ...
                if project_dir.is_dir() and not project_dir.name.startswith('.'):
                    for tissue_dir in project_dir.iterdir():
                        if tissue_dir.is_dir():
                            ...
                            if original_files:
                                original_files_found.extend([f"{project_dir.name}/{tissue_dir.name}: {f.name}" for f in original_files])
                    ...
                    for tissue_dir in project_dir.iterdir():
                        if tissue_dir.is_dir():
                            ...
                            if train_files and eval_files:
                                splits_found.append(f"{project_dir.name}/{tissue_dir.name}")
    ...
    try:
        ...
        if batch_env.exists():
                    ...
                    if project_dir.is_dir() and not project_dir.name.startswith('.'):
                        for tissue_dir in project_dir.iterdir():
                            if tissue_dir.is_dir():
                                ...
                                if batch_files:
                                    batch_files_found.extend([f"{project_dir.name}/{tissue_dir.name}: {f.name}" for f in batch_files])
            ...
            if batch_files_found:
                ...
                print("   ⚠️  Batch correction environment found but no batch corrected files")
                print("   💡 Run: source activate_batch.sh && python run_timeflies.py batch-correct")
        else:
            print("   ⚠️  No batch correction environment")
            print("   💡 Optional: Re-run setup_dev_env.sh and choose 'y' for batch environment")
    ...
    print("\n📋 Workflow status:")
    print("   1. ✅ Data files placed" if workflow_complete else "   1. ❌ Place original H5AD files in data/[project]/[tissue]/")
    print("   2. ✅ Test fixtures created" if workflow_complete else "   2. ❌ Run: python run_timeflies.py create-test-data")
    print("   3. ✅ Data splits created" if workflow_complete else "   3. ❌ Run: python run_timeflies.py setup")
    print("   4. ✅ Verification complete" if all_passed and workflow_complete else "   4. ❌ Fix issues above")
    ...
    logging.getLogger('timeflies.shared.core.active_config').setLevel(active_config_level)
    logging.getLogger('timeflies.projects.fruitfly_aging.core.config_manager').setLevel(config_manager_level)
...
def setup_command(args) -> int:
    ...
    try:
        ...
        print(f"📊 Split parameters: {split_size} cells per eval set, random_state={random_state}")
                ...
                if project_dir.is_dir() and not project_dir.name.startswith('.'):
                    for tissue_dir in project_dir.iterdir():
                        if tissue_dir.is_dir():
                            original_files = list(tissue_dir.glob("*_original.h5ad"))
                            if original_files:
                                projects_found.append((project_dir.name, tissue_dir.name))
        
        if not projects_found:
            print("❌ No projects with original data found in data/ directory")
            print("💡 Place your *_original.h5ad files in data/[project]/[tissue]/ first")
            return 1
        ...
        for project_name, tissue in projects_found:
            ...
            try:
                ...
                if project_config_path.exists():
                    ...
                    encoding_var = project_data.get('data', {}).get('encoding_variable', 'age')
                    print(f"🎯 Stratification variable: {encoding_var}")
                else:
                    print(f"⚠️  Config not found for {project_name}, using default: age")
                    encoding_var = 'age'
            except Exception as e:
                print(f"⚠️  Could not load config for {project_name}, using default: age")
                encoding_var = 'age'
            ...
            print(f"📄 Found {len(original_files)} original + {len(batch_files)} batch files")
            if len(original_files) > 0 and len(batch_files) == 0:
                print(f"   ℹ️  No batch corrected files found - only splitting original data")
        ...
        print(f"{'='*60}")
        print(f"📊 Processed {total_processed} file(s) across {len(projects_found)} project(s)")
...
def process_data_splits(file_path: Path, file_type: str, split_size: int, encoding_var: str, random_state: int) -> bool:
    ...
    try:
        ...
        if train_file.exists() or eval_file.exists():
            ...
            print(f"   📁 Found: {train_file.name if train_file.exists() else ''} {eval_file.name if eval_file.exists() else ''}")
            return True
        ...
        celltype_cols = ['cell_type', 'celltype', 'Cell_Type', 'cluster', 'leiden', 'louvain']
        ...
        if not stratify_cols:
            ...
            if len(stratify_cols) == 1:
                ...
                stratify_labels = adata.obs[stratify_cols].astype(str).agg('_'.join, axis=1)
        ...
        if split_size >= total_cells:
            print(f"   ⚠️  Split size ({split_size}) >= total cells ({total_cells}), using 20% split")
            test_size = 0.2
        else:
            test_size = split_size / total_cells
        
        print(f"   ✂️  Split ratio: {test_size:.3f} ({int(total_cells * test_size)} eval cells)")
...
def batch_command(args) -> int:
    ...
    if not BATCH_CORRECTION_AVAILABLE:
        ...
        print("Make sure you're in the batch correction environment: source activate_batch.sh")
        return 1
    ...
    try:
        ...
        if args.original:
            print("WARNING: Using --original flag for batch correction can cause data leakage!")
            print("Batch correction should only be trained on training data, not the full dataset.")
...
def create_project_test_data(project, tissue, data_file, batch_file=None, n_samples=100, n_genes=500, seed=42):
    ...
    try:
        ...
        if batch_file and batch_file.exists():
            try:
                ...
                batch_cell_indices = np.random.choice(adata_batch_full.n_obs, batch_n_cells, replace=False)
                batch_gene_indices = np.random.choice(adata_batch_full.n_vars, batch_n_genes, replace=False)
                
                adata_batch = adata_batch_full[batch_cell_indices, batch_gene_indices].copy()
                ...
                print(f"    ⚠️  Batch data processing failed: {e}, skipping batch-corrected test data")
        
        # Generate statistics
        stats = generate_data_stats(adata_subset, project, tissue, batch_corrected=adata_batch is not None)
...
def generate_data_stats(adata, project, tissue, batch_corrected=False):
    ...
    stats['expression_stats'] = {
        ...
        'non_zero_mean': float(np.mean(X_array[X_array > 0])) if np.any(X_array > 0) else 0.0
    }
    ...
    if hasattr(adata.var, 'columns'):
        stats['gene_info'] = {
            ...
            'n_highly_variable': int(adata.var.get('highly_variable', pd.Series([False]*adata.n_vars)).sum())
        }
