...
class DataSetupManager:
    ...
    def _generate_filename(self, tissue: str, file_type: str, batch: bool = False) -> str:
        """
        ...
        project = getattr(self.config.data, 'project', 'fruitfly_aging').replace('fruitfly_', '')
    ...
    def download_data(self):
        ...
        data_dir = Path(f"data/{getattr(self.config.data, 'project', 'fruitfly_aging')}/{tissue}")
        ...
        logger.warning(f"Data download not implemented. Please ensure {original_file} exists.")
    ...
    def stratified_split(self):
        ...
        logger.info(f"Split completed: {len(adata_train)} train, {len(adata_eval)} eval samples")
        return True
    ...
    def verify_splits(self):
        ...
        if train_samples + eval_samples != total_samples:
            raise ValueError(f"Sample count mismatch: {train_samples} + {eval_samples} != {total_samples}")
        ...
        encoding_var = self.config.data.encoding_variable
        original_dist = adata_original.obs[encoding_var].value_counts(normalize=True).sort_index()
        train_dist = adata_train.obs[encoding_var].value_counts(normalize=True).sort_index()
        eval_dist = adata_eval.obs[encoding_var].value_counts(normalize=True).sort_index()
        ...
        for label in original_dist.index:
            ...
            if abs(train_prop - orig_prop) > tolerance or abs(eval_prop - orig_prop) > tolerance:
                logger.warning(f"Distribution difference for {label}: orig={orig_prop:.3f}, train={train_prop:.3f}, eval={eval_prop:.3f}")
