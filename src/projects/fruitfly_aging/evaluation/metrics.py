...
class AgingMetrics:
    ...
    def evaluate_age_prediction(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        """
    ...
    def evaluate_sex_specific_performance(self, 
                                        y_true: np.ndarray, 
                                        y_pred: np.ndarray,
                                        sex_labels: np.ndarray) -> Dict[str, Any]:
        """
        ...
        if 'male_metrics' in results and 'female_metrics' in results:
            results['sex_comparison'] = {
                'mae_difference': results['male_metrics']['mae'] - results['female_metrics']['mae'],
                'r2_difference': results['male_metrics']['r2_score'] - results['female_metrics']['r2_score'],
                'correlation_difference': results['male_metrics']['pearson_correlation'] - results['female_metrics']['pearson_correlation']
            }
    ...
    def evaluate_trajectory_quality(self, trajectory_results: Dict) -> Dict[str, float]:
        ...
        metrics['significant_gene_percentage'] = (len(significant_trajectories) / len(all_trajectories)) * 100
        ...
        if method == 'linear':
            ...
            increasing_count = sum(1 for t in significant_trajectories.values() if t['correlation'] > 0)
            ...
            metrics['increasing_percentage'] = (increasing_count / len(significant_trajectories)) * 100 if len(significant_trajectories) > 0 else 0
            ...
            trajectory_types = [t['trajectory_type'] for t in significant_trajectories.values()]
            type_counts = {}
            for ttype in ['linear_increasing', 'linear_decreasing', 'u_shaped', 'inverted_u']:
                count = trajectory_types.count(ttype)
                type_counts[f'{ttype}_count'] = count
                type_counts[f'{ttype}_percentage'] = (count / len(trajectory_types)) * 100 if len(trajectory_types) > 0 else 0
            
            metrics.update(type_counts)
        
        return metrics
    
    def evaluate_aging_marker_performance(self, 
                                        adata,
                                        aging_markers: List[str],
                                        age_column: str = 'age') -> Dict[str, Any]:
        """
        ...
        results['missing_markers'] = [gene for gene in aging_markers if gene not in adata.var.index]
        ...
        for marker in available_markers:
            ...
            expression = adata.X[:, marker_idx].toarray().flatten() if hasattr(adata.X, 'toarray') else adata.X[:, marker_idx]
            ...
            for age in adata.obs[age_column].unique():
                ...
                age_groups[str(age)] = {
                    ...
                    'cv': float(np.std(age_expression) / np.mean(age_expression)) if np.mean(age_expression) > 0 else 0
                }
        ...
        if marker_analysis:
            ...
            results['summary'] = {
                ...
                'strong_age_correlation_count': sum(1 for c in correlations if abs(c) > 0.5),
                'significant_correlation_count': sum(1 for m in marker_analysis.values() if m['correlation_p_value'] < 0.05)
            }
        
        return results
    
    def _evaluate_age_group_consistency(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        """
    ...
    def calculate_aging_score(self, adata, 
                            aging_markers: List[str],
                            age_column: str = 'age') -> np.ndarray:
        """
