...
class AgingVisualizer:
    ...
    def _setup_plotting_style(self):
        ...
        plt.style.use('seaborn-v0_8-whitegrid' if hasattr(plt.style, 'seaborn-v0_8-whitegrid') else 'seaborn-whitegrid')
        sns.set_palette("husl")
    
    def plot_age_progression(self, adata, 
                           genes: List[str], 
                           age_column: str = 'age',
    ...
    def plot_sex_aging_comparison(self, adata,
                                genes: List[str],
                                age_column: str = 'age', 
                                sex_column: str = 'sex',
                                save_path: Optional[Path] = None) -> plt.Figure:
            ...
            for sex in ['male', 'female']:
                ...
                for age in ages:
                    ...
                    if len(age_sex_data) > 0:
                        expr = age_sex_data.X[:, gene_idx].toarray().flatten() if hasattr(age_sex_data.X, 'toarray') else age_sex_data.X[:, gene_idx]
                        sex_ages.append(age)
                        sex_expression.append(np.mean(expr))
                        sex_std.append(np.std(expr))
                
                # Plot with error bars
                axes[i].errorbar(sex_ages, sex_expression, yerr=sex_std, 
    ...
    def plot_aging_markers_heatmap(self, adata,
        ...
        if not available_markers:
            ...
            ax.text(0.5, 0.5, 'No aging markers found in data',
        ...
        for i, marker in enumerate(available_markers):
            ...
            for j, age in enumerate(ages):
                ...
                if len(age_data) > 0:
                    expr = age_data.X[:, marker_idx].toarray().flatten() if hasattr(age_data.X, 'toarray') else age_data.X[:, marker_idx]
    ...
    def plot_trajectory_genes(self, trajectory_results: Dict,
                            adata,
                            age_column: str = 'age',
                            top_n: int = 6,
                            save_path: Optional[Path] = None) -> plt.Figure:
        ...
        if not significant_trajectories:
            fig, ax = plt.subplots(figsize=(8, 6))
            ax.text(0.5, 0.5, 'No significant trajectories found',
                   ha='center', va='center', transform=ax.transAxes, fontsize=16)
            return fig
        ...
        if method == 'linear':
            sorted_genes = sorted(significant_trajectories.items(), 
                                key=lambda x: x[1]['p_value'])[:top_n]
        else:
            sorted_genes = sorted(significant_trajectories.items(),
                                key=lambda x: x[1]['r_squared'], reverse=True)[:top_n]
        ...
        for i, (gene, trajectory_data) in enumerate(sorted_genes):
            ...
            for age in ages:
                age_data = adata[adata.obs[age_column] == age]
                if len(age_data) > 0:
                    expr = age_data.X[:, gene_idx].toarray().flatten() if hasattr(age_data.X, 'toarray') else age_data.X[:, gene_idx]
