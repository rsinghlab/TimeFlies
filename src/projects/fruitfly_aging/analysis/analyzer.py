...
class AgingAnalyzer:
    ...
    def analyze_sex_specific_aging(self, adata, 
                                 age_column: str = 'age',
                                 sex_column: str = 'sex') -> Dict:
        """
    ...
    def identify_aging_trajectories(self, adata, 
                                  age_column: str = 'age',
        ...
        for gene_idx, gene in enumerate(adata.var.index):
            ...
            if method == 'linear':
                ...
                r_squared = 1 - (np.sum((expression - fit) ** 2) / np.sum((expression - np.mean(expression)) ** 2))
        ...
        if method == 'linear':
            significant_genes = {k: v for k, v in trajectories.items() if v['p_value'] < 0.05}
        else:
            significant_genes = {k: v for k, v in trajectories.items() if v['r_squared'] > 0.1}
    ...
    def _differential_expression_age_groups(self, adata, age1: int, age2: int, age_column: str) -> Dict:
