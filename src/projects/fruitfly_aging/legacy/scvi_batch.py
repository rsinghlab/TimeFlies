...
class BatchCorrector:
    ...
    def save_results(self, out_dir=None):
        ...
        if self.use_original:
            self.adata_original.write_h5ad(
                os.path.join(out_dir, "fly_original_batch.h5ad")
            )
        ...
        if raw_path is None:
            if self.use_original:
                raw_path = os.path.join(
                    self.base_dir, self.tissue, "uncorrected", "fly_original.h5ad"
                )
                out_prefix = "fly_original"
            else:
                raw_path = os.path.join(
                    self.base_dir, self.tissue, "uncorrected", "fly_eval.h5ad"
                )
                out_prefix = "fly_eval"
