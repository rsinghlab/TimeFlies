"""Fruit Fly Alzheimer's project-specific configuration management."""
...
class Config:
    """Configuration class with nested attribute access for alzheimer's project."""
...
class ConfigManager:
    """Fruit Fly Alzheimer's project-specific configuration manager."""
    
    def __init__(self, config_path: Optional[str] = None):
        """
        Initialize configuration manager for alzheimer's project.
        
        Args:
            config_path: Path to YAML configuration file
        """
        self.project_name = "fruitfly_alzheimers"
    ...
    def _find_config_path(self, config_path: Optional[str]) -> str:
        """Find configuration file path for alzheimer's project."""
        if config_path:
        ...
        # Try to find default config file in project-specific locations
        default_paths = [
            f"configs/{self.project_name}/default.yaml",
            f"../configs/{self.project_name}/default.yaml", 
            f"../../configs/{self.project_name}/default.yaml",
            os.path.join(os.path.dirname(__file__), f"../../../configs/{self.project_name}/default.yaml")
        ]
        ...
        raise ConfigurationError(f"No configuration file found for {self.project_name} project. Please provide a valid config path.")
        ...
        try:
            ...
            if base_config_path:
                ...
                if os.path.exists(base_config_full_path):
                    ...
                    logger.warning(f"Base config file not found: {base_config_full_path}")
            ...
            logger.info(f"Loaded {self.project_name} configuration from {self.config_path}")
            
        except Exception as e:
            raise ConfigurationError(f"Failed to load {self.project_name} config from {self.config_path}: {e}")
    
    def _merge_configs(self, base_dict: Dict[str, Any], override_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def validate_config(self) -> None:
        ...
        # Alzheimer's project specific validations
        if hasattr(self._config, 'data'):
            ...
            if not hasattr(self._config.data, 'project'):
                ...
                logger.warning(f"Config project '{self._config.data.project}' doesn't match expected '{self.project_name}'")
                self._config.data.project = self.project_name
    ...
    def save_config(self, config: Config, output_path: str) -> None:
        ...
        try:
            ...
            raise ConfigurationError(f"Failed to save {self.project_name} config to {output_path}: {e}")


def get_config(config_path: Optional[str] = None) -> Config:
    """
    Get global configuration instance for alzheimer's project (singleton pattern).
